...
.main_loop:
    ...
    ; 握手未完成时不读 stdin（数据留在管道缓冲区，先等服务器 HELLO）
...
.no_strip:
    ...
    mov     r8d, ecx                ; 保存 text_len（r8 在 parse 后不再使用）
    lea     rdi, [g_frame_buf + FRAME_HDR + NONCE_LEN]
    push    rdi                     ; 保存 payload 起始地址（用于计算长度）
    ; 复制 to
    mov     rsi, r9
    mov     ecx, r10d
    rep     movsb
    ...
    ; 复制 text
    mov     rsi, r11
    mov     ecx, r8d                ; 恢复 text_len（第一次 rep movsb 已覆盖 ecx）
    rep     movsb
    ; payload_len = rdi - payload_start（ELF64 下不能用 lea rax,[rdi-relocatable_sym]）
    pop     rax                     ; rax = payload 起始地址
    neg     rax
    add     rax, rdi                ; rax = rdi - start = payload_len
...
.check_logout:
    ...
    ; LOGOUT → 发送空 payload 加密帧
    xor     eax, eax            ; payload len = 0
    mov     r15d, PKT_LOGOUT
    jmp     .send_encrypted
...
.send_encrypted:
    ...
    mov     r13d, eax               ; 保存 payload 长度（chacha20_encrypt 会覆盖 eax）
...
%endif
    ...
    mov     eax, r13d               ; 恢复被 chacha20_encrypt 覆盖的 payload 长度
    add     eax, NONCE_LEN
...
; ================================================================
section .text   ; 切回 text 段（exit_ok 是可执行代码，不能留在 data 段）
; ================================================================
.exit_ok:
    pop     r15
    pop     r14
