...
chacha20_block:
    PROLOGUE
%ifdef _WIN32
    ; 保存 Win64 callee-saved 寄存器（PROLOGUE 已 sub rsp,32，push 后 rsp 继续下移）
    push    r12
    push    r13
    push    r14
    push    r15
    push    rbx
    ; 保存参数到 PROLOGUE shadow 区（rbp-relative，不受后续 rsp 变化影响）
    mov     [rbp - 8],   A1     ; out
    mov     [rbp - 16],  A2     ; key
    mov     [rbp - 24],  A3     ; nonce
    mov     [rbp - 32],  A4d    ; counter
    ; state 分配在 push 之后，避免与保存区重叠
    sub     rsp, 128
%else
    ; 保存 Linux callee-saved 寄存器
    push    r12
    push    r13
    push    r14
    push    r15
    push    rbx
    ; A1-A4 (rdi/rsi/rdx/rcx) 不被 push 破坏，直接搬入保留寄存器
    mov     r12, A1             ; out
    mov     r13, A2             ; key
    mov     r14, A3             ; nonce
    mov     r15d, A4d           ; counter
    ; state 分配在 push 之后（[rsp+0..127] 不与保存区重叠）
    sub     rsp, 128
%endif
    ...
    ; state 在 [rsp+0..63]，init_state 副本在 [rsp+64..127]

%ifdef _WIN32
    ; 从 PROLOGUE shadow 区加载参数到保留寄存器
    mov     r12, [rbp - 8]      ; out
    mov     r13, [rbp - 16]     ; key
    mov     r14, [rbp - 24]     ; nonce
    mov     r15d, [rbp - 32]    ; counter
%endif
    ; Linux: r12-r15 已在上方直接赋值，无需再次加载
...
.write_out:
    ...
    ; EPILOGUE：用 rbp-relative mov 恢复保留寄存器，EPILOGUE 宏负责重置 rsp
%ifdef _WIN32
    ; Win64: PROLOGUE sub rsp,32 后 push 5 个 = rbp-40..rbp-72
    mov     r12, [rbp - 40]
    mov     r13, [rbp - 48]
    mov     r14, [rbp - 56]
    mov     r15, [rbp - 64]
    mov     rbx, [rbp - 72]
%else
    ; Linux: PROLOGUE 无额外分配，push 5 个 = rbp-8..rbp-40
    mov     r12, [rbp - 8]
    mov     r13, [rbp - 16]
    mov     r14, [rbp - 24]
    mov     r15, [rbp - 32]
...
%else
    ...
    push    r14
    push    r15
    sub     rsp, 64 + 24        ; keystream(64) + nonce12(16) + padding(8) = 88
    ; 注：必须分配足够空间，不能让 nonce12 区 [rsp+64..rsp+79] 覆盖 callee-saved [rbp-40]
    mov     [rbp - 48], rdi     ; out
    mov     [rbp - 56], rsi     ; in
    mov     [rbp - 64], rdx     ; len
    mov     [rbp - 72], rcx     ; key
    mov     [rbp - 80], r8      ; nonce ptr
    mov     [rbp - 88], r9      ; ctr
%endif
...
%endif
    mov     dword [r10], 0
    mov     rax, [rbx]              ; 解引用 nonce 指针，取 8 字节 nonce 实际内容
    mov     [r10 + 4], rax
